...
void LCDScrollLeft(){
	LCDWriteCmd(0x1A); // <==== Completar //posiblemente 0x18 y sustituir por el while de 15
	waitLCD();
}
...
//Store a new character pattern on the CGRAM;
void LCDStoreCharPattern(struct MyChar ch){
    
    int i;
    //Writtes in AC the addres addr of the CGRAM
    LCDWriteCmd(ch.addr);
    waitLCD();
    
    for(i = 0; i < sizeof(ch.pattern); i++){
        LCDWriteData(ch.pattern[i]);
    }
    //Writes in AC the addres 0 of the DDRAM
    LCDMoveHome();
}
